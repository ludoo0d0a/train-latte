...
useHead(() => ({
  title: capitalize(t('pages.tweets.title')),
  meta: [
    {
      name: 'description',
      content: t('pages.tweets.description'),
    },
  ],
}))
</script>
...
<template>
  <PageWrapper>
    <PageHeader>
      <PageTitle :text="$t('pages.tweets.title')" class="capitalize" />
    </PageHeader>
    <PageBody>
      <ContentList v-slot="{ list }" path="/tweets">
        <PageSection v-for="tweet in list" :key="tweet._path">
          <div
          ...
          >
            <div class="mt-1 text-slate-600 dark:text-gray-400 text-right">
              ...
              <Anchor
                ...
                :href="`https://www.twitter.com/${tweet.author}`"
              >
